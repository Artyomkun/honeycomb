...
namespace RealLife5D.Scenes
{
    public class MoonScene : MonoBehaviour
    {
        ...
        private IEnumerator Celebration()
        {
            ...
            Debug.Log("Куратор: Отныне ты - новый Куратор Вселенных!");
            yield return new WaitForSeconds(2f);
            
            Debug.Log("Куратор: Ты можешь создавать новые миры и управлять реальностью!");
            yield return new WaitForSeconds(2f);
        ...
        {
            // Создание нового мира (для Кураторов)
            Debug.Log("Создание нового мира...");
            
            // Здесь можно добавить логику создания мира
            // Например, генерация процедурного мира
        }
    }
}